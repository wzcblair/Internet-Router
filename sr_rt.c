...
/*---------------------------------------------------------------------
 * Method: sr_search_routing_entry(struct sr_instance *sr, 
 * 				   				   struct in_addr addr)
 * Scope:  Global
 *
 * Search in the routing table for a routing entry with longest prefix
 * match.
 *
 *---------------------------------------------------------------------*/
struct sr_rt *sr_search_routing_entry(struct sr_instance *sr, 
				      				  struct in_addr addr) {
    unsigned long longestmatchaddr = 0;
    struct sr_rt *longestmatchrt = NULL;
    struct sr_rt *rt;

	/* Iterate through the interfaces and compare the masked addresses */
    for (rt = sr->routing_table; rt != NULL; rt = rt->next) {
		if (((addr.s_addr & rt->mask.s_addr) 
	    	== (rt->dest.s_addr & rt->mask.s_addr)) 
	    	&& (longestmatchaddr < rt->mask.s_addr)) {
	    	longestmatchrt = rt;
	    	longestmatchaddr = rt->mask.s_addr;
		}
    }

    return longestmatchrt;
}
